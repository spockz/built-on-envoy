...
Base address for virtual IP allocation. May be IPv4 (e.g. "10.0.0.0", answered as an A record) or IPv6 (e.g. "fd00::", answered as an AAAA record). The address family of base_ip determines which DNS record type this domain answers.
...
String key-value pairs exposed in Envoy filter state as io.builtonenvoy.dns_gateway.metadata.\<key\>.
...
CIDR prefix length. For an IPv4 base_ip the valid range is 1-32 (a /24 gives 256 virtual IPs); for an IPv6 base_ip it is 1-128 (a /64 gives 2^64 virtual IPs). Prefixes beyond the base_ip's family bound are rejected at config load.
...
[↑ Top](#) | [↑ DNS Gateway Resolver Configuration](#resolver) {/* if .SubProperties */} {/* range .Properties */} {/* if .CustomTypes */}
...
<a id="dynamic-fault-injection"></a>
## dynamic-fault-injection Configuration

Configuration for the dynamic fault injection filter. Filter-level configuration may define endpoint selectors; per-route configuration defines direct response distributions or load-based behavior.

### diagnostic

Includes the Envoy worker index diagnostic response header.
...
[↑ Top](#) | [↑ dynamic-fault-injection Configuration](#dynamic-fault-injection) {/* if .SubProperties */}

### endpoints

List of endpoint configurations, each defining fault behavior for matched routes.
...
[↑ Top](#) | [↑ dynamic-fault-injection Configuration](#dynamic-fault-injection)



#### endpoints.load_based

Load-based response distribution that switches behavior based on RPS.
...
Response distributions. Resolution acts as both relative weight and sample count.
...
Direct load-based behavior for per-route configuration.

| | |
|---|---|
| **Type** | [#/properties/endpoints/items/properties/load_based](#dynamic-fault-injection-#/properties/endpoints/items/properties/load_based) |
| **Required** | No |
...
Selects latency sampling mode for response distributions.
...
Direct response distributions for per-route configuration.

| | |
|---|---|
| **Type** | [[]#/properties/endpoints/items/properties/responses/items](#dynamic-fault-injection-#/properties/endpoints/items/properties/responses/items) |
| **Required** | No |

[↑ Top](#) | [↑ dynamic-fault-injection Configuration](#dynamic-fault-injection) {/* if .SubProperties */} {/* range .Properties */} {/* if .CustomTypes */}
